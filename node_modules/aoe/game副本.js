        ...
        if (this.rect) {
            ...
            for (let key in this.area) {
                ...
                if (area) {
                    ...
                    ctx.font='30px serif'
                    ctx.fillText(pos,area[0]+2, area[1]+2)
                }
            }
            ctx.stroke()
            ctx.closePath()
        ...
        if(!this.sprite.curAnimation||this.sprite.curAnimation.name!=='change') {
            ...
            this.sprite.playAnimation(aniName, {repeat: 2}).then(()=> {
                if (--this.shell > 0) {
                    ...
                    this.sprite.playAnimation('change',{after:500}).then(()=> {
            ...
            glGameObjectList.filter(go=>go.tag == 'enemy').sort((a, b)=>b.sprite.scale - a.sprite.scale).some(go=> {
    ...
    start(callback) {
        ...
        this.initPromise.then(()=>{
            ...
            function step(elapse) {
                ...
                for(let i=0;i<actor.shell;i++){

                }
                if(!this._stop)requestAnimationFrame(step.bind(this))
            }
